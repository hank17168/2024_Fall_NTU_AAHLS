...
//-------------------- Address Generator(Data) ------------------
    ...
    always @* begin
        if (ap_ctrl[2] == 1'b0) begin // if FIR is executing
            ...
            //    x_cnt_tmp = x_cnt;
            if (ss_tvalid && ss_tready) begin
            	if (x_cnt != 4'd10) x_cnt_tmp = x_cnt + 1'b1;
            	else x_cnt_tmp = 4'd0;
            end
            else x_cnt_tmp = x_cnt;
        end
        else
            x_cnt_tmp = 4'd0;
    end
    ...
    // count x[t-i]    
    always @* begin
	if (ss_tvalid && ss_tready) data_A_tmp = ((tlast_cnt + 1) % 11) * 4;
    	else begin
    		if (k >= 0 && k <= 10) begin
		    if (k <= x_cnt)
...
//--------------- sm_check -----------------------
    always@(posedge axis_clk or negedge axis_rst_n) begin
	    ...
	    sm_check <= 0;
            ...
            if(ss_tvalid && ss_tready) sm_check <= 1;
            else if(sm_check == 1 && y_cnt == 5'd0) sm_check <= 2;
            else if(sm_check == 2) sm_check <= 3;
            else if(sm_check == 3) sm_check <= 4;
            else sm_check <= sm_check;
        end
    end
